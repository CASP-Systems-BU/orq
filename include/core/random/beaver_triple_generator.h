#ifndef SECRECY_BEAVER_TRIPLE_GENERATOR_H
#define SECRECY_BEAVER_TRIPLE_GENERATOR_H
...
namespace secrecy::random {
    ...
    class BeaverTripleGenerator : public CorrelationGenerator {
      ...
      OLEGenerator<T, E>* vg;
      std::variant<
          std::shared_ptr<PooledGenerator<SilentOLE<T>, T, T>>, 
      ...
      /**
       * generatorGetNext - A wrapper around getNext that works with either an OLEGenerator or a PooledGenerator.
       * @param n - The number of triples to get.
       */
      auto generatorGetNext(size_t n) {
        if (pooled) {
          return std::visit([n](auto& p) {
            return p->getNext(n);
          }, pg);
        } else {
          return vg->getNext(n);
     ...
     public:
      ...
      BeaverTripleGenerator(std::shared_ptr<PooledGenerator<OLEGenerator_t, T, T>> p, std::optional<Communicator*> _comm=std::nullopt)
          : pg(p), CorrelationGenerator(p->getRank()), comm(_comm), pooled(true) {}
      
      /**
       * reserve - A function to generate Beaver triples and store them for use later.
       * @param n - The number of triples to generate.
       */
      void reserve(size_t n) {
        if (!pooled) {
          if (getRank() == 0) {
            std::cout << "Attempted to pool triples on a triple generator without a pooled generator." << std::endl;
          }
          return;
        }
        ...
        while (remaining > MAX_TRIPLES_RESERVE_BATCH) {
          std::visit([n](auto& p) {
            p->reserve(2 * MAX_TRIPLES_RESERVE_BATCH);
          }, pg);
          remaining -= MAX_TRIPLES_RESERVE_BATCH;
        }

        if (remaining > 0) {
          std::visit([n, remaining](auto& p) {
            p->reserve(2 * remaining);
          }, pg);
        }
      }
      ...
      triple_t getNext(size_t n) {
        ...
        // in user code
        return {std::vector<vec_t>({a_}), std::vector<vec_t>({b_}),
                std::vector<vec_t>({c_})};
      }
...
#endif //SECRECY_BEAVER_TRIPLES_GENERATOR_H
