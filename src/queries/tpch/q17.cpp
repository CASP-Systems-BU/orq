...
int main(int argc, char** argv) {
    ...
    std::vector<std::string> sub_join_extra_columns = {"SumQuantity", "CountQuantity", "[AvgQuantity]"};
    SubQueryJoin.addColumns(sub_join_extra_columns, SubQueryJoin.size());
    SubQueryJoin.aggregate({ENC_TABLE_VALID, "[PartKey]"}, {
    ...
    auto result_column = PartSubQueryJoin.get_column(
        PartSubQueryJoin.open_with_schema(false), "SumExtendedPrice");
    size_t result = 0;
