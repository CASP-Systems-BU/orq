...
int main(int argc, char **argv) {
    ...
    auto NSOL = NS.inner_join(OL, {"[SuppKey]"},
    ...
    NSOL.aggregate({ENC_TABLE_VALID, "[OrderKey]", "[SuppKey]"},
                          {{"OrderCount", "OrderCount", count<A>}});
    ...
    L2.aggregate({ENC_TABLE_VALID, "[OrderKey]", "[SuppKey]"},
                        {{"[Late]", "[AnyLate]", bit_or<B>}},
                        {.do_sort = false});
    ...
    L2.aggregate({ENC_TABLE_VALID, "[OrderKey]"},
                        {
    ...
    W.aggregate({ENC_TABLE_VALID, "[Name]"},
                       {{"OrderCount", "NumWait", sum<A>}});
