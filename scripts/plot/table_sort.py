...
regex_pattern = re.compile(
    r"\[ SW\] +Table +(\w+) +([\d.]+) +sec|\[=PROFILE\] +(.+?) +([\d.]+) +sec"
)
...
with open(output_file, "w") as f:
    for i in num_columns:
        ...
        for run in range(num_runs):
            ...
            if num_parties == 1:
                command = (
                    f"{test_script} -b {batch_size} -r {input_size} -num-columns {i}"
                )
            elif local:
                command = f"mpirun -np {num_parties} {test_script} -b {batch_size} -r {input_size} -num-columns {i}"
            else:
                command = f"mpirun -np {num_parties} --host {machines} {test_script} -b {batch_size} -r {input_size} -num-columns {i}"
            ...
            f.write(
                f"Script: {test_script}, Input size: {input_size}, Run: {run + 1}\n"
            )
            ...
            for match in sort_matches:
                if match[0]:  # This means it's a sort result
                    ...
                    if not radixsort:
                        quicksort_single_profiling[i][run][profile_name] = float(
                            profile_time
                        )
                    elif radixsort:
                        radixsort_single_profiling[i][run][profile_name] = float(
                            profile_time
                        )

bitonicsort_single_averages = {
    size: sum(times) / len(times) for size, times in bitonicsort_single_times.items()
}
quicksort_single_averages = {
    size: sum(times) / len(times) for size, times in quicksort_single_times.items()
}
radixsort_single_averages = {
    size: sum(times) / len(times) for size, times in radixsort_single_times.items()
}
...
plt.plot(
    num_columns,
    [bitonicsort_single_averages[size] for size in num_columns],
    label="Bitonicsort",
    marker="o",
)
plt.plot(
    num_columns,
    [quicksort_single_averages[size] for size in num_columns],
    label="Quicksort",
    marker="o",
)
plt.plot(
    num_columns,
    [radixsort_single_averages[size] for size in num_columns],
    label="Radixsort",
    marker="o",
)
...
values = np.array(
    [
        [quicksort_single_profiling_averages[col][cat] for cat in categories]
...
values = np.array(
    [
        [radixsort_single_profiling_averages[col][cat] for cat in categories]
        for col in num_columns
    ]
)
# Create the stacked bar plot
...
with open(output_file, "w") as f:
    for i in num_columns:
        ...
        for run in range(num_runs):
            ...
            # Run the script with the input size
            if num_parties == 1:
                command = f"{test_script} -b {batch_size} -r {input_size} -num-sort-columns {i}"
            elif local:
                command = f"mpirun -np {num_parties} {test_script} -b {batch_size} -r {input_size} -num-sort-columns {i}"
            else:
                command = f"mpirun -np {num_parties} --host {machines} {test_script} -b {batch_size} -r {input_size} -num-sort-columns {i}"
            result = subprocess.run(
                shlex.split(command), capture_output=True, text=True
            )
            ...
            for match in sort_matches:
                if match[0]:  # This means it's a sort result
                    ...
                    profile_name, profile_time = match[2], match[3]
                    if not radixsort:
                        quicksort_multi_profiling[i][run][profile_name] = float(
                            profile_time
                        )
                    elif radixsort:
                        radixsort_multi_profiling[i][run][profile_name] = float(
                            profile_time
                        )


bitonicsort_multi_averages = {
    size: sum(times) / len(times) for size, times in bitonicsort_multi_times.items()
}
quicksort_multi_averages = {
    size: sum(times) / len(times) for size, times in quicksort_multi_times.items()
}
radixsort_multi_averages = {
    size: sum(times) / len(times) for size, times in radixsort_multi_times.items()
}
...
plt.plot(
    num_columns,
    [bitonicsort_multi_averages[size] for size in num_columns],
    label="Bitonicsort",
    marker="o",
)
plt.plot(
    num_columns,
    [quicksort_multi_averages[size] for size in num_columns],
    label="Quicksort",
    marker="o",
)
plt.plot(
    num_columns,
    [radixsort_multi_averages[size] for size in num_columns],
    label="Radixsort",
    marker="o",
)
...
values = np.array(
    [
        [quicksort_multi_profiling_averages[col][cat] for cat in categories]
        for col in num_columns
    ]
)
# Create the stacked bar plot
...
values = np.array(
    [
        [radixsort_multi_profiling_averages[col][cat] for cat in categories]
        for col in num_columns
    ]
)
# Create the stacked bar plot
