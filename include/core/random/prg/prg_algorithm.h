...
/**
 * @brief Base class for pseudorandom generation algorithms.
 *
...
class PRGAlgorithm {
   public:
     ...
     * @param dest The span to fill with random bytes.
     */
     ...
     *
     * @tparam T The data type to fill.
     ...
     * them to the Vector.
     *
     * @tparam T The data type for the vector elements.
   ...
   protected:
    ...
    /**
     * Returns the size of the buffer allocated to
     *  temporarily store random numbers before they are copied to Vectors.
     * @return The preferred buffer size in bytes.
     */
    virtual size_t getPreferredBufferSize() { return __DEFAULT_PRGALGORITHM_BUFFER_SIZE; }
...
/**
 * @brief Base class for deterministic PRG algorithms.
 *
...
/**
 * @brief AES-based deterministic PRG implementation.
 *
...
class AESPRGAlgorithm : public DeterministicPRGAlgorithm {
    ...
    */
     ...
     */
    void incrementNonce() override { nonce++; }

    /**
     * Generate a random AES key.
     * @param key The span to fill with the generated key.
     */
   ...
   protected:
    size_t getPreferredBufferSize() override { return __MAX_AES_QUERY_BYTES; }
...
/**
 * @brief PRG implementation using /dev/urandom.
 *
...
class DevUrandomPRGAlgorithm : public PRGAlgorithm {
   ...
   public:
    ...
    void fillBytes(std::span<uint8_t> dest) override {
        size_t bytes = dest.size();
        size_t batches =
            (bytes / MAX_DEV_URANDOM_QUERY_BYTES) + (bytes % MAX_DEV_URANDOM_QUERY_BYTES != 0);
        size_t bytes_remaining = bytes;
        size_t index = 0;
        ...
        for (size_t batch = 0; batch < batches; batch++) {
            ...
            urandom.read(reinterpret_cast<char*>(dest.subspan(index, batch_bytes).data()),
                         batch_bytes);
            bytes_remaining -= batch_bytes;
            index += batch_bytes;
        }
   ...
   protected:
    size_t getPreferredBufferSize() override { return __MAX_DEV_URANDOM_QUERY_BYTES; }
};

/**
 * @brief XChaCha20-based deterministic PRG implementation.
 *
...
class XChaCha20PRGAlgorithm : public DeterministicPRGAlgorithm {
   public:
    ...
    void fillBytes(std::span<uint8_t> dest) override {
        size_t bytes = dest.size();
        size_t batches =
            (bytes / MAX_XCHACHA20_QUERY_BYTES) + (bytes % MAX_XCHACHA20_QUERY_BYTES != 0);
        size_t bytes_remaining = bytes;
        size_t index = 0;
        for (size_t batch = 0; batch < batches; batch++) {
   ...
   protected:
    size_t getPreferredBufferSize() override { return __MAX_XCHACHA20_QUERY_BYTES; }
