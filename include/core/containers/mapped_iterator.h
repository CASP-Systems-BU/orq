...
namespace secrecy {
  ...
  requires std::convertible_to<std::iter_value_t<D>, T> && std::convertible_to<std::iter_value_t<M>, std::iter_difference_t<D>>
  ...
  public:
    ...
    MappedIterator(std::vector<T>::iterator _dataIter, std::vector<VectorSizeType>::iterator _mappingIter) :
...
static_assert(std::random_access_iterator<secrecy::MappedIterator<int, std::vector<int>::iterator, std::vector<int>::iterator>>);
