...
int main(int argc, char** argv) {
    ...
    // Round to integer since Secrecy doesn't currently support fixed/floating point operations.
    ...
    single_cout("Q11 SF " << db.scaleFactor << " | FRACTION " << FRACTION << " | DIVIDE " << DIVIDE);
    ...
    MainQuery.aggregate({ENC_TABLE_VALID, "[PartKey]"}, {
        {"Value", "SumValue", sum<A>}
    });
    ...
    auto CompoundQuery = SubQuery.inner_join(MainQuery, {}, {
        {"[SumFraction]", "[SumFraction]", copy<B>}
    });
    ...
    if (pid == 0) {
        ...
        while ((ret = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            // std::cout << "PartKey: " << sqlPartKey << " | " << "Value: " << sqlValue << std::endl;
            // std::cout << "PartKey: " << partKey[result_idx] << " | " << "Value: " << value[result_idx] << std::endl;
            // std::cout << std::endl;
        ...
        if (result_idx == 0) {single_cout("Empty result");}
