...
int main(int argc, char **argv)
{
    ...
    LineItem.aggregate({ENC_TABLE_VALID, "[SuppKey]"}, 
                                {{"Revenue", "TotalRevenue", sum<A>}});
    ...
    // 'MaxTotalRevenue' value is now in the last row of 'MaxRevenue'. Extract it and replicate it in all rows
    // 1- Extract 'MaxTotalRevenue' value
    B extracted(1);
    extracted = ((B*)LineItem["[TotalRevenue]"].contents.get())->simple_subset_reference(LineItem.size() - 1, 1, LineItem.size() - 1);
    // 2- Replicate it in all rows of 'MaxRevenue'
    *((B*)LineItem["[MaxTotalRevenue]"].contents.get()) = extracted.repeated_subset_reference(LineItem.size());
    ...
    if (pid == 0) {
        ...
        if (idx == 0) {single_cout("Empty result");}
