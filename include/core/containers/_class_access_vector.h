...
namespace secrecy {
     ...
     */
    template<typename T>
    static inline void setBitMask(T &share,
                                  const bool &value,
                                  const std::make_unsigned_t<T> &mask) {
    ...
    class Vector {
         ...
         * `this` Vector, 0 <= i < size(). (Basically parity check of each element.)
...
//        }
        ...
        inline Vector simple_subset_reference(const int _start_index,
            ...
            Vector res(std::shared_ptr<VectorDataBase<T>>(
                    new SimpleVectorData<T>(data, _start_index, std::max(0, data.get()->size() - 1), _step)
            ));
            res.reset_batch();
            return res;
        }
        ...
        Vector alternating_subset_reference(const size_t _subset_included_size,
            ...
            if (_subset_excluded_size != -1) {
                ...
                Vector res(std::shared_ptr<VectorDataBase<T>>(
                        new AlternatingVectorData<T>(data, _subset_included_size,
        ...
        Vector reversed_alternating_subset_reference(const size_t _subset_included_size,
            ...
            if (_subset_excluded_size != -1) {
                Vector res(std::shared_ptr<VectorDataBase<T>>(
                        new ReversedAlternatingVectorData<T>(data, _subset_included_size, _subset_excluded_size)
                ));
                res.reset_batch();
                return res;
            } else {
                Vector res(std::shared_ptr<VectorDataBase<T>>(
                        new ReversedAlternatingVectorData<T>(data, _subset_included_size,
                                                                    this->total_size() - _subset_included_size)
                ));
                res.reset_batch();
                return res;
        ...
        Vector cyclic_subset_reference(const size_t _subset_cycles) const {
            Vector res(std::shared_ptr<VectorDataBase<T>>(
                    new CyclicVectorData<T>(data, _subset_cycles)
            ));
            res.reset_batch();
            return res;
        }
        ...
        Vector simple_bit_compress(const size_t &start, const size_t &step, const size_t &end,
            ...
            for (size_t i = 0, j = 0; j < total_bits; i++, j += MAX_BITS_NUMBER)
            {
                auto r = &res[i];
                for(size_t k = j, p = 0;
                        p < MAX_BITS_NUMBER && k < total_bits;
                        k++, p++) {
                    setBitValue(*r,
        ...
        void simple_bit_decompress(const Vector &other,
                                   const size_t &start, const size_t &step, const size_t &end,
            ...
            for (size_t i = 0, j = 0; j < total_bits; i++, j += MAX_BITS_NUMBER)
            {
                ...
                for(size_t k = j, p = 0;
                        p < MAX_BITS_NUMBER && k < total_bits;
                        k++, p++) {
                    setBitValue((*this)[k / bits_per_element],
                        getBit(r, p),
        ...
        Vector alternating_bit_compress(const size_t &start, const size_t &step,
            ...
            const size_t bits_per_element = (MAX_BITS_NUMBER - start) / (included_size + excluded_size) * bits_per_chunk;
            const size_t last_chunk_bits_per_element = std::min(included_size, ((MAX_BITS_NUMBER - start) % (included_size + excluded_size))) / step;
            ...
            for (size_t i = 0; i < total_bits; ++i) {
                ...
                const size_t element_bit_index = start + direction_offset + element_chunk_index * (included_size + excluded_size) + ((i % total_bits_per_element) % bits_per_chunk) * step * direction;
        ...
        inline Vector alternating_bit_compress(const size_t &start, const size_t &step,
                                               const size_t &included_size, const size_t &excluded_size) const {
...
//                        new std::vector<T>(args...))) { batch_end = data.get()->size(); }
        ...
        Vector(size_t _size, T _init_val = 0) :
                data(std::shared_ptr<VectorDataBase<T>>(
                        new VectorData<T>(_size, _init_val))) {
         ...
         * Applies the move assignment operator to T. Assigns the contents of the `other` vector to the this
         ...
         * Applies the copy assignment operator to T. Copies the contents of the `other` vector to this vector.
...
//            static const int MAX_BITS_NUMBER = std::pow(2, std::ceil(std::log2(std::numeric_limits<T>::digits)));
//            Vector res(n);
...
//        }
        ...
        bool same_as(const Vector<T>& other) const {
            if (this->size() != other.size()) {
                ...
                std::cout << "[same_as]: size mismatch: this size " << this->size() << " != " << other.size() << "\n";
#endif
                return false;
            }
