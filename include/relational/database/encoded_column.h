...
// Macro for column-constant operations, where we need to convert the constant
// into a (temporary, public-shared) column
#define binary_op_element(_op_, eType, vType)                                                     \
    std::unique_ptr<EncodedColumn> operator _op_(const int64_t & other) const {                   \
        ...
        vType<Share, EVector> v2 =                                                                \
            secrecy::service::runTime->public_share<EVector::replicationNumber>(v2_);             \
    ...
    std::unique_ptr<EncodedColumn> operator _op_(const int64_t & other) const {                \
        ...
        if (encoding == AShared) {                                                             \
            ...
            ASharedVector<Share, EVector> ka =                                                 \
                secrecy::service::runTime->public_share<EVector::replicationNumber>(k_);       \
...
// Macro for column-constant operations, where the underlying shared vector op
// expects a constant; e.g. shifts and public division.
...
// NOTE: Although these operator templates are defined in secrecy, we must redefine them in
// secrecy::relational
