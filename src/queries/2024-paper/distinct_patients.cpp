 ...
 * @brief Query to find distinct patients who have been diagnosed with "hd" and prescribed "aspirin".
 * @date 2024-10-22
 * 
 * SQL: 
 *   SELECT
...
size_t demographicSize(const double scaleFactor) {return std::round(scaleFactor * DEMOGRAPHIC_MULTIPLIER);}
size_t diagnosisSize(const double scaleFactor) {return std::round(scaleFactor * DIAGNOSIS_MULTIPLIER);}
size_t medicationSize(const double scaleFactor) {return std::round(scaleFactor * MEDICATION_MULTIPLIER);}
...
EncodedTable<T> getDemographicTable(const double scaleFactor, sqlite3 *sqlite_db) {
    ...
    if (sqlite_db != nullptr) {
        ...
        if (ret != SQLITE_OK) {single_cout("Create error (Demographic): " << sqlite3_errmsg(sqlite_db))}
...
EncodedTable<T> getDiagnosisTable(const double scaleFactor, sqlite3 *sqlite_db) {
    ...
    if (sqlite_db != nullptr) {
        ...
        ret = sqlite3_exec(sqlite_db, sqlCreate, 0, 0, NULL);
        if (ret != SQLITE_OK) {single_cout("Create error (Diagnosis): " << sqlite3_errmsg(sqlite_db))}
...
EncodedTable<T> getMedicationTable(const double scaleFactor, sqlite3 *sqlite_db) {
    ...
    if (sqlite_db != nullptr) {
        ...
        ret = sqlite3_exec(sqlite_db, sqlCreate, 0, 0, NULL);
        if (ret != SQLITE_OK) {single_cout("Create error (Medication): " << sqlite3_errmsg(sqlite_db))}
