...
int main(int argc, char** argv) {
    ...
    while (num_left > 0) {
        ...
        runTime->randomnessManagers[0]->getCorrelation<int32_t, Correlation::BeaverMulTriple>()->getNext(amount);
        num_left -= amount;
    }
    ...
    if (bitwidth == 32) {
        ...
        while (num_left > 0) {
            ...
            // read the triples to free the memory
            runTime->randomnessManagers[0]->getCorrelation<int64_t, Correlation::BeaverAndTriple>()->getNext(amount);
            num_left -= amount;
        }
    } else if (bitwidth == 64) {
        num_left = num_ands;
        while (num_left > 0) {
            ...
            // read the triples to free the memory
            runTime->randomnessManagers[0]->getCorrelation<int32_t, Correlation::BeaverAndTriple>()->getNext(amount);
            num_left -= amount;
        }
    }
