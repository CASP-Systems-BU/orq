...
template <typename T>
void test_pairwise(int test_size) {
    Vector<T> v(test_size);
    std::iota(v.begin(), v.end(), 0);
    BSharedVector<T> input = secret_share_b(v, 0);

    input.shuffle();
    orq::operators::pairwise_sort(input);
    auto asc_open = input.open();

    input.shuffle();
    orq::operators::pairwise_sort(input, SortOrder::DESC);
    auto desc_open = input.open();

    assert(std::is_sorted(asc_open.begin(), asc_open.end()));
    std::reverse(desc_open.begin(), desc_open.end());
    assert(std::is_sorted(desc_open.begin(), desc_open.end()));
}
...
int main(int argc, char** argv) {
    ...
    test_pairwise<int64_t>(1024);
    test_pairwise<int64_t>(60000);
    single_cout("Pairwise 64...OK");
