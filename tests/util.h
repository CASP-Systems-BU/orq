...
constexpr bool is_power_of_two(uint64_t x) { return (1 << std::bit_width(x - 1)) == x; }

template <typename T>
bool same_elements(orq::Vector<T> x, orq::Vector<T> y) {
...
#define ASSERT_POWER_OF_TWO(x)                                                            \
    {                                                                                     \
        ...
        if (!is_power_of_two(x)) {                                                        \
            std::cerr << "assertion failed: " #x " (= " << (s) << ") not power of two\n"; \
            assert(false);                                                                \
        }                                                                                 \
    }
