#ifndef SECRECY_NULL_SERVICE_SETUP_H
#define SECRECY_NULL_SERVICE_SETUP_H
...
namespace secrecy::service::null_service {

namespace {
    template <typename PRG = PRGAlgorithm,
              typename PermGen = HMShardedPermutationGenerator>
    auto setup_random_generation() {
        ...
        std::map<std::tuple<std::type_index, Correlation>, secrecy::random::CorrelationGenerator*>
            cg;

        // Setup dummy common PRG. Use seed of all zeros.
        auto zero_seed = std::vector<unsigned char>(crypto_aead_aes256gcm_KEYBYTES);
        std::unique_ptr<DeterministicPRGAlgorithm> prg_algorithm = std::make_unique<secrecy::random::AESPRGAlgorithm>(zero_seed);
        auto commonPRG = std::make_shared<secrecy::random::CommonPRG>(std::move(prg_algorithm), 0);
...
namespace plaintext_1pc {
    ...
    init_mpc_system(secrecy::NullCommunicator, secrecy::random::PRGAlgorithm,
                    secrecy::Plaintext_1PC, secrecy::Plaintext_1PC_Factory);
    ...
    static void secrecy_init(int argc, char** argv) {
        ...
        secrecy::benchmarking::stopwatch::partyID = 0;
        ...
        for (int i = 0; i < runTime->get_num_threads(); ++i) {
            ...
            runTime->workers[i].attach(std::make_unique<secrecy::NullCommunicator>(),
...
namespace dummy_0pc {
    ...
    init_mpc_types(int, secrecy::Vector, std::vector, secrecy::EVector, 1);
    init_mpc_system(secrecy::NullCommunicator, secrecy::random::ZeroRandomGenerator,
                    secrecy::Dummy_0PC, secrecy::Dummy_0PC_Factory);
    init_mpc_functions(1);
    ...
    static void secrecy_init(int argc, char** argv) {
        ...
        for (int i = 0; i < runTime->get_num_threads(); ++i) {
            ...
            runTime->workers[i].attach(
                std::make_unique<secrecy::NullCommunicator>(),
