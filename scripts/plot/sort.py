...
with open(output_file, "w") as f:
    for size in input_sizes:
        ...
        for run in range(num_runs):
            ...
            if num_parties == 1:
                command = f"{test_script} -b {batch_size} -r {size}"
            elif local:
                command = (
                    f"mpirun -np {num_parties} {test_script} -b {batch_size} -r {size}"
                )
            else:
                command = f"mpirun -np {num_parties} --host {machines} {test_script} -b {batch_size} -r {size}"
...
quicksort_averages = {
    size: sum(times) / len(times) for size, times in quicksort_times.items()
}
radixsort_averages = {
    size: sum(times) / len(times) for size, times in radixsort_times.items()
}
# Calculate average profile data
profile_averages = {
    size: {name: sum(times) / len(times) for name, times in profiles.items()}
...
plt.plot(
    input_sizes,
    [quicksort_averages[size] for size in input_sizes],
    label="Quicksort",
    marker="o",
)
plt.plot(
    input_sizes,
    [radixsort_averages[size] for size in input_sizes],
    label="Radixsort",
    marker="o",
)
...
def prepare_plot_data(sort_averages, profile_averages, profile_components):
    ...
    profile_times = [
        profile_averages[size].get(component, 0) for component in profile_components
    ]
...
quicksort_profile_times = prepare_plot_data(
    quicksort_averages, profile_averages, profile_components_quicksort
)
radixsort_profile_times = prepare_plot_data(
    radixsort_averages, profile_averages, profile_components_radixsort
)
...
quicksort_percentages = [
    f"{(time / sum(quicksort_profile_times)) * 100:.1f}%"
    for time in quicksort_profile_times
]
...
axs[0].legend(
    ...
    [
        f"{comp}: {perc}"
        for comp, perc in zip(profile_components_quicksort, quicksort_percentages)
    ],
    title="Quicksort Components",
    loc="center left",
    bbox_to_anchor=(1, 0, 0.5, 1),
)

# Calculate percentages for radixsort
radixsort_percentages = [
    f"{(time / sum(radixsort_profile_times)) * 100:.1f}%"
    for time in radixsort_profile_times
]
...
axs[1].legend(
    ...
    [
        f"{comp}: {perc}"
        for comp, perc in zip(profile_components_radixsort, radixsort_percentages)
    ],
    title="Radixsort Components",
    loc="center left",
    bbox_to_anchor=(1, 0, 0.5, 1),
)
