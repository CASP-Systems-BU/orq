...
/**
 * @brief Performs conditional swap on two boolean shared vectors.
 *
 * Swaps elements between x and y based on comparison result (x >= y).
 * After the swap, x contains the smaller values and y contains the larger values.
 *
...
/**
 * @brief Performs odd-even merge on a boolean shared vector.
 *
 * Implements the odd-even merge algorithm.
 * The vector size must be a power of two.
 *
 * @tparam Share The underlying data type of the shared vectors.
 * @tparam EVector Share container type.
 *
 * @param v Vector to be merged (modified in place).
 */
template <typename Share, typename EVector>
...
namespace {
    ...
    /**
     * @brief Merges two sorted vectors of an std::vector into a single sorted std::vector.
     *
     * Note: Plain text bitonic merge for implementation reference.
     *
     * @tparam Share The underlying data type of the shared vectors.
    ...
    std::vector<int> bitonicMergePlainText(const std::vector<int>& v1, const std::vector<int>& v2) {
        ...
        merged.insert(merged.end(), v2.rbegin(),
                      v2.rend());  // Reverse v2 to make it decreasing -> increasing
    ...
    /**
     * @brief Reverses the second half of the columns and data vectors.
     *
...
/**
 * @brief Sorts vectors based on some keys.
 *
...
/**
 * @brief Sorts a vector that has two already sorted halves in the same order.
 *
 * @tparam Share The underlying data type of the shared vectors.
 * @tparam EVector Share container type.
