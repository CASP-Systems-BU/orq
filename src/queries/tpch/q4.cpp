...
int main(int argc, char** argv) {
    ...
    // count(*) as order_count
    OrderLineItemJoin.aggregate({ENC_TABLE_VALID, "[OrderPriority]"}, {
    ...
    OrderLineItemJoin.deleteColumns({
        "[CommitDate]", "[OrderDate]", "[OrderKey]", "[ReceiptDate]"
    });
    ...
    if (pid == 0) {
        ...
        while ((ret = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            // single_cout("Priority " << sqlPriority << " -> SQL: " << sqlCount << " | Query: " << resultMap[sqlPriority]);
