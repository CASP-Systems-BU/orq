...
ALL_QUERIES = [f"Q{i}" for i in range(1, 22 + 1)]
ALL_QUERIES.extend(sorted(NAME_MAP.values()))
...
def _configure_ticks(ax):
    ax.set_xticks(list(range(len(ALL_QUERIES))) + [-1])
    ax.set_xticklabels(ALL_QUERIES + ["TPCH"])
...
def plot_sublot(ax, p, lan, wan):
    ...
    if not has_lan and not has_wan:
        print(f"Skipping {p} - no data")
        # configure ticks anyway
        _configure_ticks(ax)
        return
    ...
    if has_wan:
        ax.bar(wan['query'], wan["median"], hatch=WAN_STYLE, color=c)
    if has_lan:
        ax.bar(lan['query'], lan["median"], hatch=LAN_STYLE, color=c)
    ...
    # Q6 clarification
    q6_lan_val = lan[lan["query"] == "Q6"]["median"].values if has_lan else [np.nan]
    q6_wan_val = wan[wan["query"] == "Q6"]["median"].values if has_wan else [np.nan]
    ...
    # ax.set_xticks(list(range(22)) + [-1])
    # ax.set_xticklabels([f"Q{i}" for i in range(1, 22 + 1)] + ["TPCH"])
    _configure_ticks(ax)
...
if __name__ == "__main__":
    ...
    # Directory paths
    present = list(filter(None, [args.L2, args.L3, args.L4, args.W2, args.W3, args.W4]))
    if not present:
        print("Must provide at least one data directory!")
        exit(1)

    result_dir = Path(present[0]).parent.parent.expanduser()
