#ifndef SECRECY_ENCODED_TABLE_H
#define SECRECY_ENCODED_TABLE_H
...
using namespace secrecy::benchmarking::utils;
...
class EncodedTable {
     ...
     *
     * @param mask_column the column to use as the mask
   ...
   public:
     ...
     *
     * @param shuffle whether to shuffle the table before revealing.
     ...
     *
     * NOTE: The default column encoding is A-shared. To create a B-shared
...
#ifndef DEBUG_SKIP_EXPENSIVE_TABLE_OPERATIONS
        secrecy::aggregators::aggregate(keys_vec, b_agg, a_agg, dir, table_id_vec);
#else
        single_cout("...skipped");
#endif
    ...
    void head(size_t n) {
        if (n > this->size()) {
            std::cerr << "warning: taking head(" << n
                      << ") of table "
                         " of size "
                      << this->size() << "\n";
        }
    ...
    void tail(const size_t n) {
        if (n > this->size()) {
            std::cerr << "warning: taking tail(" << n
                      << ") of table "
                         " of size "
                      << this->size() << "\n";
        }
    ...
    EncodedTable concatenate(EncodedTable &other, bool power_of_two = false) {
        ...
        B secretSharedOne(1);
        secrecy::operators::secret_share_vec(one, secretSharedOne);
        // Set them all to 1
    ...
    /**
     * @brief inner join between this table and `right`. assumes primary
    ...
    EncodedTable _join(EncodedTable &right, std::vector<std::string> keys, AggregationSpec agg_spec,
                       JoinOptions opt) {
        PRINT_TABLE_INSTRUMENT("[TABLE_JOIN] L=" << size() << " R=" << right.size() << " k="
                                                 << keys.size() << " a=" << agg_spec.size());
        ...
        // table.
        EncodedTable concat = this->concatenate(right, false);
        ...
        if (opt.left_outer) {
            if (opt.right_outer) {
                ...
                // (a) and (b)
                concat["[##VALID_TEMP]"] = concat[ENC_TABLE_VALID] &
                                           ~(*(concat[ENC_TABLE_JOIN_ID] & concat[ENC_TABLE_UNIQ]));
                auto n = concat.size();
                // (c)
        ...
        if (opt.trim_invalid && !opt.left_outer) {
            ...
            auto w = std::numeric_limits<std::make_unsigned_t<Share>>::digits;
     ...
     * @param right
     * @param keys
     * @param spec Aggregation specification used for listing columns to
    ...
    static std::vector<std::shared_ptr<EncodedColumn>> secret_share(
        const std::vector<secrecy::Vector<T>> &columns, const std::vector<std::string> &schema,
   ...
   private:
     ...
     * @param column  column name to copy
