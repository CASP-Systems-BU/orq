...
int main(int argc, char** argv) {
    ...
    Customers.project({"[CustKey]", "[C_Name]", "[AcctBal]", "[Phone]", "[Address]", "[Comment]", "[NationKey]"});
    ...
    auto CustomerOrderLineItemJoin = Customers.inner_join(OrderLineItemJoin, {"[CustKey]"}, {
    ...
    auto result = FinalJoin.aggregate({ENC_TABLE_VALID, "[CustKey]"},
        {{"Revenue", "Revenue", sum<A>}}
    );
