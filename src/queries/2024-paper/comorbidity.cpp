...
#ifdef SECRECY_QUERY
// When running the Secrecy comparison, we use a different size to match them.
#define COHORT_MULTIPLIER (256)
#define DIAGNOSIS_MULTIPLIER (COHORT_MULTIPLIER * 8192)
#else
...
EncodedTable<T> getCohortTable(const double scaleFactor, sqlite3 *sqlite_db) {
    ...
    if (sqlite_db != nullptr) {
        ...
        if (ret != SQLITE_OK) {single_cout("Create error (Cohort): " << sqlite3_errmsg(sqlite_db))}
...
int main(int argc, char** argv) {
    ...
    FilteredDiagnosis.aggregate({ENC_TABLE_VALID, "[diag]"}, {
	    {"DiagCount", "DiagCount", count<A>}
    });
