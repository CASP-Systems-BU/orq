...
class Worker {
   public:
    ...
    secrecy::random::RandomnessManager* getRandManager() const {
        assert(rand_ != nullptr);
        return this->rand_.get();
    }
    secrecy::Communicator* getCommunicator() const {
    ...
    void attach(std::unique_ptr<Communicator>&& c,
                std::unique_ptr<secrecy::random::RandomnessManager>&& r) {
        assert(c != nullptr && r != nullptr);
        comm_ = std::move(c);
        rand_ = std::move(r);
    }
     ...
     * @param r 
     */
    void attach_rand(std::unique_ptr<secrecy::random::RandomnessManager>&& r) {
        assert(r != nullptr);
        rand_ = std::move(r);
    }
   ...
   private:
    ...
    // ...randomness manager...
    std::unique_ptr<secrecy::random::RandomnessManager> rand_;
    // ...and task queue.
