...
int main(int argc, char** argv) {
    ...
    // Now S contains one row per id along with the total number of occurrences of this id in the original S
    S.addColumns({"CNT"}, S.size());
    S.aggregate({ENC_TABLE_VALID, "[id]"}, {
        {"CNT", "CNT", count<A>}
    });

    // Semi-join S with R and copy S[CNT] to R for each match.
    // Now R contains partial aggregates
    auto RJ = S.inner_join(R, {"[id]"},{
        {"CNT", "CNT", copy<A>}
    });
    RJ.project({"[ak]", "CNT"});
    ...
    // Open (R.ak, R[CNT])
    RJ.aggregate({ENC_TABLE_VALID, "[ak]"}, {
        {"CNT", "CNT", sum<A>}
    });
