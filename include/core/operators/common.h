...
// Forward declarations to avoid undefined-symbol issues
...
namespace aggregators {
    ...
    /**
     * @brief Denotes the direction of an aggregation or prefix network.
     *
     */
...
namespace operators {
     ...
     */
    template <typename Share, typename EVector>
    static void auto_conversion(BSharedVector<Share, EVector>& x,
     ...
     */
    template <typename Share, typename EVector>
    static void auto_conversion(ASharedVector<Share, EVector>& x,
    ...
    static void secret_share_vec(const Vector<Share>& data, BSharedVector<Share, EVector>& out,
        ...
        out = orq::service::runTime->secret_share_b_internal<EVector::replicationNumber>(
            data, data_party);
    }
    ...
    static void secret_share_vec(const Vector<Share>& data, ASharedVector<Share, EVector>& out,
                                 const PartyID data_party = 0) {
        out = orq::service::runTime->secret_share_a_internal<EVector::replicationNumber>(
            data, data_party);
    }
}  // namespace operators
}  // namespace orq
