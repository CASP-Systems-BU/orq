#ifndef SECRECY_DEBUG_H
#define SECRECY_DEBUG_H
...
///////////////////////////////////////
// Update default communicator here: //
#ifndef COMMUNICATOR_NUM
#define COMMUNICATOR_NUM MPI_COMMUNICATOR
#endif
///////////////////////////////////////

// If SINGLE_EXECUTION is defined, don't compile with a communicator. (Useful
// for debugging.)
...
#define NOCOPY_COMMUNICATOR_THREADS -1  // Set to '-1' to use 1 comm thread per Secrecy thread
#endif
////////////////////////////////////////
...
// Whether we should run with a LAN (default) or WAN configuration.
// This currently affects the default batch size and adder circuit, but we may
// make future changes.
...
// See docs/vector-mappings.md for more details
...
// Whether to use the original protocol of Dalskov, et al., or our custom
// optimized version.
...
// To run on a multi-node cluster, set this define to be the cluster-routable
// hostname of the server (party 0). For example, on CloudLab, this should be
// "node0"
#ifndef LIBOTE_SERVER_HOSTNAME
#define LIBOTE_SERVER_HOSTNAME "localhost"
#endif

// Print relevant statistics for a protocol
...
#define SERVICE_NAMESPACE secrecy::service::mpi_service
#elif MPC_USE_NO_COPY_COMMUNICATOR
#define SERVICE_NAMESPACE secrecy::service::nocopy_service
#endif
...
#define SERVICE_NAMESPACE secrecy::service::null_service
...
#ifndef MPC_USE_MPI_COMMUNICATOR
#define MPI_Finalize()
#endif

#define single_cout(x)                                                            \
    {                                                                             \
        if (secrecy::service::runTime->getPartyID() == 0) std::cout << x << "\n"; \
    }
#define all_cout(x)                                                                        \
    {                                                                                      \
        std::cout << "[P" << secrecy::service::runTime->getPartyID() << "] " << x << "\n"; \
    }
...
#define DEBUG_PRINT(x) secrecy::debug::print(#x, x)

// Compiler hack to print a type via compiler error. Just do print_type<X>.
// Useful for figuring out complex type aliases.
template <typename>
struct print_type;

// custom 128 bit handler
