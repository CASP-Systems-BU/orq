...
int main(int argc, char** argv) {
    ...
    L_alice.project({"[ShipDate]", "[OrderKey]", "[ReturnFlag]", "ExtendedPrice", "Discount", "Tax", "Quantity"});
    ...
    auto L = L_bob.uu_join(L_alice, {"[OrderKey]"}, {
        {"[LineStatus]", "[LineStatus]", copy<B>}
    });
    ...
    L.deleteColumns(
        {"Quantity", "Discount", "ExtendedPrice", "Charge", "DiscPrice"});
