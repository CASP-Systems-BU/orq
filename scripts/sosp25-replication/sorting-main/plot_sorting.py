...
def plot(data_2pc_quick, data_2pc_radix, data_3pc_quick, data_3pc_radix, data_4pc_quick, data_4pc_radix, x_values):
    ...
    size_labels = [f"$2^{{{x}}}$" for x in range(20, x_values[-1] + 1)]
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print('Usage: python plot_sorting.py <home-directory> <max-pow-2>')
        exit()

    data_dir = sys.argv[1] + '/results/sorting-main/'
    ...
    x_values = list(range(20, int(sys.argv[2]) + 1))
