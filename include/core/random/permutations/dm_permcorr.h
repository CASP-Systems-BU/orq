#ifndef SECRECY_DM_PERMUTATION_CORRELATION_GENERATOR_H
#define SECRECY_DM_PERMUTATION_CORRELATION_GENERATOR_H
...
/**
 * Real Dishonest Majority Permutation Correlation Generator
 */
template <typename T>
class DMPermutationCorrelationGenerator : public DMShardedPermutationGenerator<T> {
   ...
   public:
    ...
    DMPermutationCorrelationGenerator(
        int _rank,
        int thread,
    ...
    /**
     * queryRandomOracle - Query the random oracle (hash function)for a given number of hashes.
    ...
    Vector<__uint128_t> queryRandomOracle(int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (crypto_generichash(out_ptr, sizeof(__int128_t), data, sizeof(i), key, key_size) != 0) {
    ...
    void getNext(std::shared_ptr<ShardedPermutation> perm) {
        ...
        if (DMBase::getRank() == 0) {
            ...
            Vector<__int128_t> A_1 = oprf->evaluate_plaintext<__int128_t>(hashes_0.as_std_vector(), key);
            
            // permute the hashes
            std::vector<int> pi_vec(n);
            gen_perm(pi_vec, prg);
            Vector<int> pi_0(std::move(pi_vec));
            secrecy::operators::local_apply_perm_single_threaded(hashes_1, pi_0);
            ...
            // perform the plaintext PRF evaluations
            OPRF::key_t key = oprf->keyGen();
            Vector<__int128_t> A_0 = oprf->evaluate_plaintext<__int128_t>(hashes_1.as_std_vector(), key);

            // permute the hashes
            std::vector<int> pi_vec(n);
            gen_perm(pi_vec, prg);
            Vector<int> pi_1(std::move(pi_vec));
            secrecy::operators::local_apply_perm_single_threaded(hashes_0, pi_1);
...
#endif // SECRECY_DM_PERMUTATION_CORRELATION_GENERATOR_H