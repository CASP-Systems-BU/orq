...
if [ ! -d "$HOME/flamegraph" ]; then
    ...
    sudo apt -qy install linux-tools-common linux-tools-generic linux-tools-$(uname -r)
fi
...
EXEC_STRING="./$EXEC -t $THREADS -b $BATCH $EXTRA_ARGS"
...
mpirun -n 1 perf record -F 101 -g -- $EXEC_STRING : \
