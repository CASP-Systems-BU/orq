...
class DMShardedPermutation : public ShardedPermutation {
   ...
   public:
    ...
    DMShardedPermutation(size_t n)
        : perm(std::make_shared<dm_perm_t>(n, n, n, n)),
          encoding(orq::Encoding::BShared),
          common_prg(nullptr),
          has_common_prg(false) {}
     ...
     */
    DMShardedPermutation(size_t n, orq::Encoding _encoding)
        : perm(std::make_shared<dm_perm_t>(n, n, n, n)),
          encoding(_encoding),
          common_prg(nullptr),
          has_common_prg(false) {}
     ...
     * @param _encoding The encoding type of the permutation correlation.
     */
    DMShardedPermutation(dm_perm_t _perm, orq::Encoding _encoding)
        : perm(std::make_shared<dm_perm_t>(_perm)),
          encoding(_encoding),
          common_prg(nullptr),
          has_common_prg(false) {}
     ...
     * @return A shared pointer to the underlying data.
     */
    std::shared_ptr<dm_perm_t> getTuple() { return perm; }

    /**
     * Expose the type of the correlation through a getter function.
     * @return The encoding type of the permutation correlation.
     */
    orq::Encoding getEncoding() { return encoding; }

    /**
     * Expose the existence of a CommonPRG through a getter function.
     * @return True if a CommonPRG is set, false otherwise.
     */
    bool hasCommonPRG() { return has_common_prg; }

    /**
     * Expose the CommonPRG through a getter function.
     * @return A shared pointer to the CommonPRG.
     */
    std::shared_ptr<CommonPRG> getCommonPRG() { return common_prg; }

    /**
     * Set the CommonPRG.
     * @param _common_prg The CommonPRG to set.
     */
     ...
     */
    size_t size() { return std::get<0>(*perm).size(); }
    ...
    std::shared_ptr<ShardedPermutation> clone() {
        ...
        return std::make_shared<DMShardedPermutation<T>>(
            std::make_tuple(std::move(pi_copy), std::move(A_copy), std::move(B_copy),
                            std::move(C_copy)),
            encoding);
    }
};
...
/**
 * Convert a DMShardedPermutation from one type to another.
 * @param perm The permutation to convert.
 * @return A new permutation with converted type.
 */
template <typename T1, typename T2>
std::shared_ptr<DMShardedPermutation<T2>> dm_perm_convert_type(
    std::shared_ptr<DMShardedPermutation<T1>> perm) {
...
/**
 * Convert a DMShardedPermutation from binary to arithmetic sharing.
 * @param perm The binary shared permutation to convert.
 * @return A new arithmetic shared permutation.
 */
template <typename T>
std::shared_ptr<DMShardedPermutation<T>> dm_perm_convert_b2a(
    std::shared_ptr<DMShardedPermutation<T>> perm) {
...
class DMShardedPermutationGenerator : public ShardedPermutationGenerator {
   ...
   public:
     ...
     * @param _rank The rank of the current party.
    ...
    /**
     * Allocate memory for many DMShardedPermutations so they can be passed to and
     ...
     * @param perm The permutation to verify.
     */
    void assertCorrelated(std::shared_ptr<DMShardedPermutation<T>>& perm) {
        ...
        bool is_a_shared = perm->getEncoding() == orq::Encoding::AShared;
     ...
     */
    Communicator* getComm() const { return comm.has_value() ? comm.value() : nullptr; }
};
