#ifndef SECRECY_SERVICE_RUNTIME_H
#define SECRECY_SERVICE_RUNTIME_H
...
// In WAN, we need to minimize rounds
#define DEFAULT_BATCH_SIZE -1
#else
// The default batch size for normal batching, if not specified by the user
// From 2024-Nov experiments on AWS, -12 looks to be a good default batch size.
...
// (Soon to be replaced by more templates)
...
#define define_1_alloc(S, F, InT, OutT)                                             \
    ...
    OutT F(InT x, const T &...args) {                                               \
        return eval_protocol_1arg_alloc<RepProto<S, R>, &Worker::PROTO_OBJ_NAME(S), \
...
#define runtime_declare_protocol_functions(T)                                              \
    ...
    define_reshare(T);                                                                     \
    define_1_arg(T, reconstruct_from_a, secrecy::Vector<T>, std::vector<EVectorClass(T)>); \
    define_1_arg(T, reconstruct_from_b, secrecy::Vector<T>, std::vector<EVectorClass(T)>); \
    define_1_alloc(T, open_shares_a, EVectorClass(T), secrecy::Vector<T>);                 \
    define_1_alloc(T, open_shares_b, EVectorClass(T), secrecy::Vector<T>);                 \
    define_1_alloc(T, secret_share_a, secrecy::Vector<T>, EVectorClass(T));                \
    define_1_alloc(T, secret_share_b, secrecy::Vector<T>, EVectorClass(T));                \
    define_1_alloc(T, public_share, secrecy::Vector<T>, EVectorClass(T));                  \
...
class RunTime {
   private:
    // Use a barrier to synchronization workers with
    // main thread
   ...
   public:
    ...
    // TEMPORARY: external parties sometimes need to access stuff in (a) thread
    // so we give them access only to worker 0.
    // Currently only used in shuffle cost model... tbd
    Worker *worker0;

    secrecy::Communicator *comm0() { return worker0->getCommunicator(); }

    secrecy::random::RandomnessManager *rand0() { return worker0->getRandManager(); }
    ...
    void modify_parallel_2arg(E1 &x, const E2 &y, E1 &(E1::*func)(const E2 &)) {
        ...
        addTask(x.total_size(), [&](const size_t start, const size_t end) {
            return std::make_unique<Task_1_ref<E2, E1>>(
                y, x, start, end, batch_size,
                [&, this](E2 &_y, E1 &_x) { (_x.*func)(_y); });
        });
    ...
    /**
     * generate_parallel - Generates randomness in parallel using all available
     * threads
     *
     ...
     *
     * TODO: what do the rest of the arguments mean?
     */
    template <typename InputType, typename... T>
    void generate_parallel(void (secrecy::random::RandomnessManager::*func)(InputType &, T...),
    ...
    /**
     * generate_permutations - Generate a set of sharded permutations in
    ...
    void generate_permutations(
        std::vector<std::shared_ptr<secrecy::random::ShardedPermutation>> &ret) {
        thread_stopwatch::InstrumentBlock _ib{};
        ...
        using permVec_t = std::vector<std::shared_ptr<secrecy::random::ShardedPermutation>>;
        ...
        for (int t = 0; t < num_threads; ++t) {
            ...
            auto generator =
                w.getRandManager()
                    ->getCorrelation<T, secrecy::random::Correlation::ShardedPermutation>();
    ...
    // Function to make the batch size divisible by some number
    ...
    void populateLocalRandom(Vector<T> &v) {
        generate_parallel(&secrecy::random::RandomnessManager::generate_local, v);
    }
    ...
    void populateCommonRandom(Vector<T> &v, std::set<int> group) {
        generate_parallel(&secrecy::random::RandomnessManager::generate_common, v, group);
    }

    /**
     * reserve_triples - Generates and pools Beaver triples in parallel.
     ...
     */
    template <typename T>
    void reserve_triples(void (secrecy::random::RandomnessManager::*func)(size_t n), size_t n) {
        thread_stopwatch::InstrumentBlock _ib{};
    ...
    /**
     * reserve_mul_triples - Calls reserve_triples() to generate arithmetic triples.
     * @param n - The number of triples to generate.
     */
    template <typename T>
    void reserve_mul_triples(size_t n) {
#ifdef MPC_PROTOCOL_BEAVER_TWO
        reserve_triples<T>(&secrecy::random::RandomnessManager::reserve_mul_triples<T>, n);
#endif
    }

    /**
     * reserve_mul_triples - Calls reserve_triples() to generate binary triples.
     * @param n - The number of triples to generate.
     */
    template <typename T>
    void reserve_and_triples(size_t n) {
#ifdef MPC_PROTOCOL_BEAVER_TWO
        reserve_triples<T>(&secrecy::random::RandomnessManager::reserve_and_triples<T>, n);
#endif
    }
    ...
    /**
     * print_communicator_statistics - Print the number of bytes sent by each communicator.
     */
...
#endif  // SECRECY_SERVICE_RUNTIME_H
