...
#ifdef SECRECY_QUERY
// When comparing with secrecy, diag is 2M
#define DIAGNOSIS_SIZE (2000 * 1000)
#else
   ...
   public:
    ...
    EncodedTable<T> getCreditScoreTable() {
        ...
        if (this->sqlite_db) {
            ...
            ret = sqlite3_exec(this->sqlite_db, sqlCreate, 0, 0, NULL);
            if (ret != SQLITE_OK) {
                single_cout("SQL error on CreditScore (Create): " << sqlite3_errmsg(this->sqlite_db))
            }
            ...
            for (size_t i = 0; i < S; ++i) {
                ...
                ret = sqlite3_step(stmt);
                if (ret != SQLITE_DONE) {
                    single_cout("SQL error on CreditScore (Insert): " << sqlite3_errmsg(this->sqlite_db))
                }
                sqlite3_reset(stmt);
            }
            ...
            ret = sqlite3_exec(this->sqlite_db, "COMMIT;", 0, 0, NULL);
            if (ret != SQLITE_OK) {
                single_cout("SQL error on CreditScore (Commit): " << sqlite3_errmsg(this->sqlite_db))
            }
        } else {
            single_cout("SQLite setup skipped")
        }
