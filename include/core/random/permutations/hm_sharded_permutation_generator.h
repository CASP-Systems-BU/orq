#ifndef SECRECY_HM_SHARDED_PERMUTATION_GENERATOR_H
#define SECRECY_HM_SHARDED_PERMUTATION_GENERATOR_H
...
namespace secrecy::random {
    ...
    public:
        ...
        HMShardedPermutation(size_t _size) : perm(std::make_shared<std::map<Group, LocalPermutation>>()), m_size(_size) {}

        /**
         * getPermMap - Expose the underlying permutation map.
         */
        std::shared_ptr<std::map<Group, LocalPermutation>> getPermMap() {
            return perm;
        }
    ...
    void gen_perm(std::vector<int>& permutation, std::shared_ptr<CommonPRG> generator) {
        ...
        secrecy::Vector<uint32_t> random(_buffer_size);
    ...
    public:
         ...
         */
        HMShardedPermutationGenerator(int _rank,
                                       std::shared_ptr<CommonPRGManager> _commonPRGManager,
                                       std::vector<std::set<int>> _groups)
            : ShardedPermutationGenerator(_rank), rank(_rank), commonPRGManager(std::move(_commonPRGManager)), groups(_groups){}
        ...
        /**
         * allocate - Allocate memory for many HMShardedPermutations so they can be passed to and generated by the runtime.
        ...
        /**
         * generateBatch - Generate a set of permutations for the given size.
        ...
        void generateBatch(std::vector<std::shared_ptr<ShardedPermutation>>& ret) {
            ...
            for (std::shared_ptr<ShardedPermutation> _perm : ret) {
                std::shared_ptr<HMShardedPermutation> perm = std::dynamic_pointer_cast<HMShardedPermutation>(_perm);
                // generate random permutations for each group
                for (auto group : groups) {
                    if (!group.contains(rank)) {
                        continue;
...
#endif // SECRECY_HM_SHARDED_PERMUTATION_GENERATOR_H