...
/**
 * @brief Silent Oblivious Linear Evaluation generator.
 *
 * @tparam T The data type for the correlation elements
 */
template <typename T>
class GilboaOLE : public OLEGenerator<T, Encoding::AShared> {
#ifdef USE_LIBOTE
   ...
   public:
    GilboaOLE(int rank, Communicator *comm, int thread)
    ...
    /**
     * Generate silent OLE correlations.
