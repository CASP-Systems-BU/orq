 ...
 * @param perm1 The first permutation in the pair.
 * @param perm2 The second permutation in the pair.
 */
template <typename T>
...
/**
 * @brief Singleton class for managing permutation correlations.
 *
 * Manages queues of individual permutations and pairs of permutations for efficient
 * batch generation and retrieval.
 */
class PermutationManager {
    ...
    std::queue<std::pair<std::shared_ptr<ShardedPermutation>, std::shared_ptr<ShardedPermutation>>>
        pair_queue;
   ...
   public:
     ...
     */
    size_t size_pairs() { return pair_queue.size(); }
    ...
    void reserve(size_t size_permutation, size_t num_permutations, size_t num_pairs = 0) {
        ...
        if (((!queue.empty()) || (!pair_queue.empty())) && (stored_size != size_permutation)) {
            single_cout("WARNING: Emptying " << queue.size()
     ...
     * @param dm_encoding The encoding type for the permutation.
     * @return The next sharded permutation in the queue.
     */
    template <typename T>
    std::shared_ptr<ShardedPermutation> getNext(
        size_t size_permutation,
        std::optional<orq::Encoding> dm_encoding = orq::Encoding::BShared) {
        if (runTime->getNumParties() == 2) {
            stopwatch::profile_preprocessing();
        }
        ...
        if (runTime->getNumParties() == 2) {
            ...
            if (dm_encoding.value() == orq::Encoding::AShared) {
     ...
     * @param size_permutation The size of the permutation to get.
     * @param dm_encoding_1 The encoding type for the first permutation.
     * @param dm_encoding_2 The encoding type for the second permutation.
    ...
    std::pair<std::shared_ptr<ShardedPermutation>, std::shared_ptr<ShardedPermutation>> getNextPair(
        size_t size_permutation,
        std::optional<orq::Encoding> dm_encoding_1 = orq::Encoding::BShared,
        std::optional<orq::Encoding> dm_encoding_2 = orq::Encoding::BShared) {
        if (runTime->getNumParties() == 2) {
            stopwatch::profile_preprocessing();
        }
        ...
        if (dm_encoding_1.value() == orq::Encoding::AShared) {
            result_1 = dm_perm_convert_b2a(converted_1);
        }

        if (dm_encoding_2.value() == orq::Encoding::AShared) {
