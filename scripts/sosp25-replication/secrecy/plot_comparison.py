...
with open('../../../results/secrecy/secrecy/secrecy.csv') as f:
    nsdi_times = list(map(float, f.read().strip().split(',')))
...
with open('../../../results/secrecy/orq/orq.csv') as f:
    current_times = list(map(lambda p: float(p.strip()), f.read().strip().split(',')))
################################################

print(len(nsdi_times), nsdi_times)
print(len(current_times), current_times)

################################################
# Reorder data
################################################

experiments = experiments_order
################################################

for e, t1, t2 in zip(experiments, nsdi_times, current_times):
    print(f"{e}\t{t1:8}\t{t2:8}")
